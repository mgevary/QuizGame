...
| `p2p.js` WebRTC | QR handshake (offer QR → answer QR) | same WiFi / hotspot | yes | **yes** |
| `lan.js` + `scripts/lan-server.mjs` | 4-digit room code | same WiFi, a laptop hosts | yes | **yes** |
| `lan.js` + Cloudflare Durable Object (`worker/`) | 4-digit room code | anywhere | no | written, not deployed |

All four drive the one coordinator, which is the point of keeping it free of
network and DOM. `scripts/mptest.mjs` proves the room path with two real
browsers and a real server.
...
- Big screen on a **laptop** → `npm run lan`, everyone opens the printed address and joins with
